...
STAGING_OUTPUT_PATH = f"s3a://{S3_BUCKET}/silver/markPriceKlines/"
...
def get_last_processed_time(interval_name):
    ...
    try:
        table_exists = con.execute("SELECT count(*) FROM information_schema.tables WHERE table_name = 'fact_markpriceklines_features'").fetchone()[0]
        if table_exists == 0: return None
        query = f"SELECT MAX(close_time) FROM fact_markpriceklines_features WHERE interval = '{interval_name}'"
        ...
        con.close()
def calculate_markpriceklines_features(spark, interval_name, interval_window_ms):
    print(f"\n📈 Processing MarkPriceKlines: {interval_name} (window: {interval_window_ms / 1000 / 60} minutes)")
    ...
    input_path = f"s3a://{S3_BUCKET}/silver/ohlc_mark/"
    # Đọc dữ liệu từ MinIO bronze (đã nén jsonl.gz, partitioned date/hour)
    df = spark.read.parquet(input_path)

    df = df.filter(F.col("channel") == f"mark-price-candle{interval_name}")
...
def mark_price(spark):
    ...
    print("STARTING MarkIndexKline BATCH PIPELINE...")
    for name, window_ms in intervals:
        has_data = calculate_markpriceklines_features(spark, name, window_ms)
        if has_data:
